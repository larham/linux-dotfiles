...
3. `run_onchange_after_25-install-firefox-apt.sh.tmpl` — Installs native Firefox .deb from official Mozilla APT repo, replacing Snap (Linux only)
4. `run_once_after_30-disable-sleep.sh.tmpl` — Disables sleep, suspend, and lid switch; enables SSH (Linux only)
5. `run_onchange_after_40-gnome-favorites.sh.tmpl` — Pins Ghostty, Firefox, and GNOME System Settings to GNOME dock (Linux only)
6. `run_onchange_after_50-macos-keymap.sh.tmpl` — Maps Left Command key to Control in GNOME for macOS-style Cmd+C / Cmd+V shortcuts (Linux only)
...
├── run_onchange_after_25-install-firefox-apt.sh.tmpl # Installs native Firefox .deb via Mozilla APT repo (Linux only)
